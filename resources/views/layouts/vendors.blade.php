@foreach ($items as $item)
    @if ($type == 'relationable')
        <div class=""
            ...
            <div style="width: 100% !important; align-items: center;  display:flex;  justify-content: space-between;"
                ...
                <div class="">
                    <a
                        @if ($vendor->hasActiveStory($item->id) == 1) href='{{ route('vendor.home', ['vendor' => $item->name, 'fId' => $item->id]) }}' @endif>
                        ...
                        <img style=" margin:5px 5px 0px 5px; border-radius: 50%; border:1px solid black; padding:3px; object-fit: cover; max-width:100px; height: 100px;"
                            ...
                            alt="" /></a>
                    <div class="vendorBoxItem   " title="امتیازدهی به فروشگاه">
                ...
                <div style="width: 70%; height: 100%; " class="">
    ...
    @else
        <div class="VEndorItem VEndorItemMain bg-white">
            ...
            <div class="vendorBox ">
                ...
                <div class="">
                    ...
                    <a
                        @if ($item->hasActiveStory($item->id) == 1) href='{{ route('vendor.home', ['vendor' => $item->name, 'fId' => $item->id]) }}' @endif>
                        <img @if ($item->hasActiveStory($item->id) == 1) style="border:2px solid Red;"

                     onclick="openStory({{ $item->id }} , 0 );" @endif
                            class="
                ...
                <div style=" " class="vendorBoxCenterDiv ">
                    <div class="vendorDetail">


                        <a href="{{ route('vendor.home', ['vendor' => $item->name]) }}"
                            class="vendorBoxItem vendorMame col-3 dastnevis " style="font-size: 31px;">
                            {{ $item->title }}
                        </a>
                        ...
                        <div class="mt-4 vendorDetail numandAdd d-flex justify-content-evenly" style="">
                            ...
                            <div class="phonenumber-button">
                                <span id="phoneSvg({{ $item->id }})"
                                    class=" vendorBoxItem btn btn-outline-primary"
                                    onclick=" $(this).css('display' , 'none');        $(this).next().css('display' , 'block')"
                            ...
                            <div>
                                @auth
                                    ...
                                    @if (loginedUserFollwingVendorStatus($item->id) == 0)
                                        <form action="{{ route('user.follow.follow') }}" method="post"
                                            class="text-center">
                                            @csrf
                                    ...
                                    @elseif(loginedUserFollwingVendorStatus($item->id) == 1)
                                        <form action="{{ route('user.follow.follow') }}" method="post"
                                            ...
                                            <button type="submit" class="btn btn-outline-primary" style="margin:8px;">
                                                دنبال
                                                کردن متقابل </button>
                            ...
                            <div class="row store-icons  d-none d-md-block">
                                ...
                                @auth
                                    ...
                                    <a @if (count(App\Models\favorite::where('vendor_id', Auth::user()->vendor->id)->where('vendor_iddd', $item->id)->get()) < 1) style="display:none" @endif
                                        class="vendorliked mt-2" href="#" title="حذف از علاقه مندی ها"
                                        onclick="removeFromFavorite('v',{{ $item->id }}); window.location.reload(); ">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                                            fill="red" class="bi bi-heart-fill heart-style" viewBox="0 0 16 16">
                                            <path fill-rule="evenodd"
                                                d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" />
                                        </svg>
                                    </a>
                                ...
                                @guest
                                    ...
                                    @if ($counter)
                                        <a class="vendorliked" href="#" title="حذف از علاقه مندی ها"
                                            onclick="removeFromFavorite('v',{{ $item->id }}); window.location.reload(); ">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                                                fill="red" class="bi bi-heart-fill heart-style" viewBox="0 0 16 16">
                                                <path fill-rule="evenodd"
                                    ...
                                    @else
                                        <a class="vendordissliked" href="#" title="افزودن به علاقه مندی ها"
                                            onclick="addToFAvorite('v' , {{ $item->id }} )">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                                                fill="currentColor" class="bi bi-heart heart-style" viewBox="0 0 16 16">
                                                <path
                                ...
                                <a onclick="copyToClipboardVendorFromAll('{{ $item->name }}')"
                                    ...
                                    <img width="30" height="30"
                                        src="{{ asset('main/tool/duuydusaduas-min.png') }}"
                    ...
                    <div class="vendorBoxItem bottum">



                        <a class=" defualtItems vendorBoxItem iransansmedium">
                            دسته بندی فعالیت :
                        </a>
                        ...
                        @foreach ($d as $ddd)
                            ...
                            @if ($category)
                                <a class=" iransanslight p-2 defualtItems  vendorBoxItem"
                                    style=" font-size: 15px; background-color:#dfdfdf; color:black; border-radius: 5px; "
                                    href="{{ route('categories.show', ['category' => $category->slug]) }}">
                                    {{ $category->name }}
                                </a>
                            @endif
