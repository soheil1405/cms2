        ...
        <div class=""
            ...
            <div style="width: 100% !important; align-items: center;  display:flex;  justify-content: space-between;"
                ...
                <div id="Address" style="floa" class="vendorBoxItem">
                    ...
                    @auth
                        @if (Auth::user()->vendor->id != $item->vendor->id)
    ...
    /* box-shadow: rgba(6, 24, 44, 0.4) 0px 0px 0px 2px, rgba(6, 24, 44, 0.65) 0px 4px 6px -1px, rgba(255, 255, 255, 0.08) 0px 1px 0px inset; */">
            ...
            <div class=" row">
                ...
                <div class="col-12 text-center">
                    ...
                    @if ($item->hasActiveStory($item->id) == 1)
                        <a

                        
                            href='{{ route('products.show', ['vendor' => $item->name, 'product' => $product->slug, 'urlback' => url()->full(), 'fId' => $item->id, 'lastOne' => $item->id]) }}'>
                        ...
                        vendorBoxProfile m-0 hoverbtn"src="{{ asset(env('VENDOR_IMAGES_AVATAR_UPLOAD_PATH') . $item->avatar) }}"
                                data-zoom-image="{{ asset(env('VENDOR_IMAGES_AVATAR_UPLOAD_PATH') . $item->name) }}"
                                alt="" />

                        </a>
                    @else
                        <img class="vendorBoxProfile m-0 hoverbtn"src="{{ asset(env('VENDOR_IMAGES_AVATAR_UPLOAD_PATH') . $item->avatar) }}"
                            data-zoom-image="{{ asset(env('VENDOR_IMAGES_AVATAR_UPLOAD_PATH') . $item->name) }}"
                            alt="" />
                    @endif
                    <a href="{{ route('vendor.home', ['vendor' => $item->name]) }}"
                        class=" vendorMame dastnevis col-3 " style="font-size: 28px;">
                        {{ $item->title }}
                    </a>

                    {{-- امتیازدهی به فروشگاه  --}}
                    {{-- <div class="vendorBoxItem   ">
                ...
                <div class="col-12 text-center pt-3">
                    <div class=" d-flex justify-content-evenly " style="">
                        ...
                        <div class="m-1">
                            <span id="phoneSvg({{ $item->id }})" class="  btn btn-outline-primary"
                                onclick=" $(this).css('display' , 'none');        $(this).next().css('display' , 'block')"
                                ; href="#" class=" vendorBox showNumBt m-1 ">
                                <span class="iransansmedium">
                                    تلفن</span>
                        ...
                        <div>
                            ...
                            @if ($item->id != Auth::user()->vendor->id)
                                ...
                                @if (loginedUserFollwingVendorStatus($item->id) == 0)
                                    <form action="{{ route('user.follow.follow') }}" method="post" class="text-center">
                                        ...
                                        <button type="submit" class="btn btn-outline-primary m-1">دنبال
                                            کردن </button>
                        ...
                        <div class="">
                            ...
                            <span id="AddressSvg "
                                onclick=" $(this).css('display' , 'none');      $(this).next().css('display' , 'block')"
                                class="showNumBt  btn btn-outline-primary m-1">
                                <span class="iransansmedium ">
                                    آدرس</span>
                        ...
                        <div class="row store-icons  d-none d-md-block">
                            ...
                            <a onclick="copyToClipboardVendorFromAll('{{ $item->name }}')">
                                {{-- <svg xmlns="http://www.w3.org/2000/svg" width="30" style="color:blue;"
                                    height="30" fill="currentColor" class="bi bi-share-fill" viewBox="0 0 16 16">
                                    <path
                                        d="M11 2.5a2.5 2.5 0 1 1 .603 1.628l-6.718 3.12a2.499 2.499 0 0 1 0 1.504l6.718 3.12a2.5 2.5 0 1 1-.488.876l-6.718-3.12a2.5 2.5 0 1 1 0-3.256l6.718-3.12A2.5 2.5 0 0 1 11 2.5z" />
                                </svg> --}}
        ...
        App\Models\favorite::where('vendor_id', Auth::user()->vendor->id)->where('vendor_iddd', $item->id)->get(),
            ...
            <a class="btn btn-primary btnGotoVendorPage w-100  mt-3"
