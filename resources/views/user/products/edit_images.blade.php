...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xl-12 col-md-12 mb-4 p-5 bg-white">
                    ...
                    <div class="col-md-3">
                        <div class="card">
                            ...
                            <div class="card-body text-center">
                                <form action="{{ route('user.products.images.destroy', ['product' => $product->id]) }}" method="post">
                                    @method('DELETE')
                                    @csrf
                                    <input type="hidden" name="image_id" value="{{ $image->id }}">
                                    <button class="btn btn-danger btn-sm mb-3" type="submit">حذف</button>
                                </form>
                                <form action="{{ route('user.products.images.set_primary', ['product' => $product->id]) }}"
                                    method="post">
                                    @method('PUT')
                                    @csrf
                                    <input type="hidden" name="image_id" value="{{ $image->id }}">
            ...
            <form action="{{ route('user.products.images.add', ['product' => $product->id]) }}" method="POST"
